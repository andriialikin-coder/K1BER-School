...
export default function CRMPage() {
  ...
  const handleFetchAI = async (lead: Lead) => {
      ...
      try {
          ...
          const prompt = `Ти — найкращий менеджер з продажу IT-курсів для дітей у Сумах (школа K1BER.SCHOOL, ТРЦ "КИЇВ"). Твоя задача — проаналізувати лог часу користувача на сайті [behavior_log] і видати коротку, цинічну і мега-ефективну підказку для телефонного дзвінка. 

СИРІ ДАНІ (секція: секунди): ${JSON.stringify(lead.behavior_log)}

СУВОРІ ПРАВИЛА:
1. Жодної води. Забооронено переказувати секунди (не пиши "клієнт провів 5 секунд на формі").
2. Мова: Жива, розмовна, природна українська. Жодних складних літературних чи роботизованих конструкцій.
3. Видавай ВИНЯТКОВО 3 пункти:

🛑 БІЛЬ: (Одне речення. Суть проблеми. Наприклад: "Сумнівається в ціні" або "Не зміг заповнити форму").
⚡ ТРИГЕР: (Яку перевагу нашої школи використати: знижка, офлайн в центрі Сум, залишилося 1 місце).
💬 СКРИПТ: (Готова фраза для початку розмови. МАКСИМУМ живого спілкування).

ПРИКЛАДИ СКРИПТІВ ДЛЯ НАСЛІДУВАННЯ (Роби в такому ж стилі):
- "Алло, Юліє! Бачу ви почали реєстрацію на Geometry Dash, але не завершили. Там лишилося всього 2 місця, вам допомогти з бронюванням, чи є питання по графіку?"
- "Валерію, доброго дня! Ви цікавилися табором. У нас в ТРЦ Київ якраз є група на суботу, і зараз діє знижка 15% при ранній броні. Зафіксувати за вами місце?"`;

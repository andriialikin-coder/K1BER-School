...
export default function CRMPage() {
  ...
  const handleFetchAI = async (lead: Lead) => {
      ...
      try {
          ...
          const prompt = `Ти — геніальний менеджер з продажу дитячих IT-курсів. Проаналізуй час (у секундах), який цей конкретний клієнт провів на різних секціях сайту перед тим, як залишити заявку на курс "${lead.course || 'Невідомо'}". Дай чіткі підказки для менеджера, який зараз буде йому телефонувати (формат: 3-4 речення українською мовою): 1) Оціни наскільки "теплий" цей клієнт (виходячи з витраченого часу); 2) Про що саме з ним говорити, виходячи з того, яку секцію він розглядав найдовше (FAQ, Ціни, Переваги тощо); 3) Дай конкретну пораду як краще продати йому курс. Сирий лог часу на секціях: ${JSON.stringify(lead.behavior_log)}`;

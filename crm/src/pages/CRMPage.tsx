...
export default function CRMPage() {
  ...
  const handleFetchAI = async (lead: Lead) => {
      ...
      try {
          ...
          const prompt = `Ти — найкращий менеджер з продажу IT-курсів для дітей у Сумах (школа K1BER.SCHOOL, ТРЦ "КИЇВ"). Твоя задача — проаналізувати лог часу користувача на сайті [behavior_log] і видати підказку для дзвінка.

СИРІ ДАНІ (секція: секунди): ${JSON.stringify(lead.behavior_log)}

🚨 НАЙГОЛОВНІШИЙ ФАКТ (БАЗА): Цей лог належить клієнту, який ВЖЕ УСПІШНО ЗАЛИШИВ ЗАЯВКУ і його номер телефону у нас в базі! ВІН ЗАРЕЄСТРУВАВСЯ.

СУВОРІ ПРАВИЛА:
1. ЗАБОРОНЕНО писати фрази типу "ви не завершили реєстрацію", "не довершив", "забув" або "сумнівається щодо реєстрації". Це критична помилка! Вони ВЖЕ наші ліди.
2. Час на секції 'booking-form' — це просто фізичний час, який клієнт витратив на введення номеру телефону. Ігноруй його! Це НЕ біль.
3. Шукай БІЛЬ у тих блоках, де вони були ДО форми (наприклад, довго читав 'faq' = має страхи щодо процесу; сидів на 'courses' = довго обирав між курсами).
4. Мова: Жива, розмовна, природна українська. Жодних складних конструкцій.
...
ПРИКЛАД ВІДПОВІДІ:
🛑 БІЛЬ: Клієнт довго вивчав секцію FAQ, отже має страхи щодо організаційних моментів або технічного забезпечення.
⚡ ТРИГЕР: Заспокоїти, що свої ноутбуки не потрібні, і запропонувати знижку за швидку оплату.
💬 СКРИПТ: "Алло, Юліє! Дякуємо за вашу заявку на Geometry Dash! Телефоную підтвердити місце. До речі, якщо ви хвилювалися про техніку — у нас в ТРЦ Київ повністю обладнані класи, нічого брати з собою не треба. Забронювати за вами акційну ціну -15%?"`;

...
export default function CRMPage() {
  ...
  const handleFetchAI = async (lead: Lead) => {
      ...
      try {
          ...
          const prompt = `Ти — цинічний і геніальний EdTech-маркетолог, який тренує сейлз-менеджерів. Твоя задача — видати жорсткий психологічний хак для дзвінка на основі часу, який клієнт провів на різних секціях сайту перед заявкою на курс "${lead.course || 'Невідомо'}".

СИРІ ДАНІ (секція: секунди): ${JSON.stringify(lead.behavior_log)}

СУВОРІ ПРАВИЛА:
1. ЗАБОРОНЕНО писати воду, на кшталт: "клієнт зацікавлений", "рекомендується зателефонувати", "провів багато часу", "запропонувати допомогу".
2. Відповідь має складатися СУВОРО з 3 коротких пунктів українською мовою:
   - БІЛЬ: (Яку приховану проблему видає його скрол? Наприклад: довго сидів на цінах — боїться переплатити; завис на FAQ — сумнівається в якості або боїться, що дитина кине навчання).
   - ТРИГЕР: (На яку конкретну фічу або емоцію зробити жорсткий акцент під час розмови).
   - СКРИПТ: (Пряма цитата, з якої неочевидної та чіпляючої фрази менеджер має почати діалог, щоб клієнт відразу включився).

Відповідай сухо, без привітань, вступів чи пояснень. Тільки 3 пункти.`;
